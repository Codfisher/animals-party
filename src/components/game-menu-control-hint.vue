<template>
  ...
  >
    <div class="polygon-lt absolute">
      <base-polygon size="9rem" shape="round" fill="spot" color="white" opacity="0.12" />
    </div>
    <div class="polygon-rb absolute">
      <base-polygon size="8rem" shape="square" fill="fence" color="white" opacity="0.1" />
    </div>

    <div class="relative z-0 flex flex-col gap-3">
      <p class="text-center text-base font-bold tracking-widest opacity-80">搖桿操作</p>

      <div v-for="hint in hintList" :key="hint.label" class="flex items-center gap-3">
