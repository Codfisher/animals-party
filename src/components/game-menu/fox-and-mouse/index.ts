...
export async function createFoxAndMouseIsland(
  scene: Scene,
  param: Param,
): Promise<FoxAndMouseIsland> {
  const { position, rotation } = defaults(param, defaultParam);

  const rootNode = new TransformNode('fox-and-mouse-island');

  function createLight() {
    const light = new HemisphericLight('fox-and-mouse-island-light', new Vector3(-5, 5, 5), scene);
    light.intensity = 0.1;
  ...
  async function createFoxes() {
    ...
    const tasks = list.map((option, i) => createFox(`fox-${i}`, scene, option));
