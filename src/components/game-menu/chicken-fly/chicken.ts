...
export async function createChicken(name: string, scene: Scene, param?: Param) {
  const { scaling, position, rotation, radius, delay } = defaults(param, defaultParam);
  ...
  function initRootNodeAnimation() {
    ...
    const midValue = fromValue.multiply(Quaternion.RotationAxis(Axis.Y, Tools.ToRadians(180)));
    const toValue = midValue.multiply(Quaternion.RotationAxis(Axis.Y, Tools.ToRadians(180)));
